...
describe("joinEvent", () => {
  ...
  it("joins an event successfully", async () => {
    ...
    const result = await joinEvent("event-1", "test-token");

    expect(global.fetch).toHaveBeenCalledWith(
      "https://test-api.com/events/join/event-1",
      {
        method: "POST",
        headers: {
  ...
  it("works with null token", async () => {
    ...
    const result = await joinEvent("event-2", null);

    expect(global.fetch).toHaveBeenCalledWith(
      "https://test-api.com/events/join/event-2",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
